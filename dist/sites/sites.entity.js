"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
const users_entity_1 = require("./../users/users.entity");
const measures_entity_1 = require("./../measures/measures.entity");
const typeorm_1 = require("typeorm");
const probes_entity_1 = require("./../probes/probes.entity");
let SiteEntity = class SiteEntity {
};
__decorate([
    typeorm_1.PrimaryColumn(),
    __metadata("design:type", Number)
], SiteEntity.prototype, "id", void 0);
__decorate([
    typeorm_1.Column({ default: 'undefined' }),
    __metadata("design:type", String)
], SiteEntity.prototype, "name", void 0);
__decorate([
    typeorm_1.OneToMany(type => probes_entity_1.ProbeEntity, probe => probe.site),
    __metadata("design:type", Array)
], SiteEntity.prototype, "probes", void 0);
__decorate([
    typeorm_1.OneToMany(type => measures_entity_1.MeasureEntity, measure => measure.site),
    __metadata("design:type", Array)
], SiteEntity.prototype, "measures", void 0);
__decorate([
    typeorm_1.ManyToMany(type => users_entity_1.UserEntity, user => user.sites),
    __metadata("design:type", Array)
], SiteEntity.prototype, "users", void 0);
SiteEntity = __decorate([
    typeorm_1.Entity('sites')
], SiteEntity);
exports.SiteEntity = SiteEntity;
//# sourceMappingURL=sites.entity.js.map